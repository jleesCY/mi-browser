...
export default function App() {
  ...
  const handleIncomingUrl = (url: string | null) => {
    ...
    if (url.startsWith('mibrowser://')) {
        ...
        if (stripped.startsWith('?url=') || stripped.startsWith('url?=')) {
             ...
             // Direct usage: mibrowser://google.com
             targetUrl = stripped;
        }
    ...
    if (targetUrl && (targetUrl.startsWith("http://") || targetUrl.startsWith("https://"))) {
      ...
      // Check if we already have this tab open to prevent duplicates from rapid clicks
  ...
  const handleShouldStartLoadWithRequest = (request: any) => {
    ...
    // --- 1. Allow Internal Blob/Data URLs (PDFs, Images generated by JS) ---
    ...
    try {
        // Special handling for Android Intents with Fallback URLs
        if (url.startsWith('intent://')) {
            Linking.openURL(url).catch(() => {
                // If the app isn't installed, look for browser_fallback_url
                // Format: intent://...#Intent;...;S.browser_fallback_url=http...;end
                const fallbackMatch = url.match(/browser_fallback_url=([^;]+)/);
                if (fallbackMatch && fallbackMatch[1]) {
                    const fallbackUrl = decodeURIComponent(fallbackMatch[1]);
                    
                    // Navigate the current tab to the fallback URL
                    setTabs((prev) =>
                        prev.map((t) => {
        ...
        // For everything else (mailto:, tel:, maps:, market:), try to open externally
